...
function generatePhasePattern(hot, max, num) {
    sample_list = []
    for (let i = 0; i < num; i++) {
        get_hot = getRandomInt(2);
        if (get_hot === 1) {
            sample_list.push(hot.toString());
        } else {
            sample_list.push((getRandomInt(max) + 1).toString());
        }
    }
    return sample_list;
}
...
element_list = []; // output
sample_list1 = generatePhasePattern(1, 3, 100); // user1 is hot
sample_list2 = generatePhasePattern(2, 3, 100); // user2 is hot
